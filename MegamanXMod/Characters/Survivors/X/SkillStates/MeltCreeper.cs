        ...
        private void FireSimpleBullet()
        {
            if (!hasFired)
            {
                ...
                if (isAuthority)
                {
                    ...
                    XBusterMediumProjectille.projectilePrefab = XAssets.MeltCreeperProjectile;
                    XBusterMediumProjectille.position = childLocator.FindChild("MeltCreeperFrontPos").transform.position;
                    XBusterMediumProjectille.rotation = childLocator.FindChild("MeltCreeperFrontPos").rotation;
                    XBusterMediumProjectille.owner = gameObject;
                    XBusterMediumProjectille.damage = 1;
                    XBusterMediumProjectille.force = force;
                    XBusterMediumProjectille.crit = RollCrit();
                    XBusterMediumProjectille.speedOverride = 2f;
