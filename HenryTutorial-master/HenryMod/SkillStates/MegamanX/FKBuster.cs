        ...
        private void FireArrowC()
        {
            if (!this.hasFired)
            {
                ...
                if (base.isAuthority)
                {
                    ...
                    ProjectileManager.instance.FireProjectile(Modules.Projectiles.shotFMJ, aimRay.origin, Util.QuaternionSafeLookRotation(aimRay.direction), base.gameObject, this.damageCoefficient * 3f * this.damageStat, 0f, Util.CheckRoll(this.critStat, base.characterBody.master), DamageColorIndex.Default, null, -1f);
                }
            }
        }
